...
STARVATION_LIMIT = 3  # cycles before a lane is forced green
...
class Intersection:
    ...
    def to_dict(self):
        return {
            ...
            "signals": [s.to_dict() for s in self.signals],  # make sure TrafficSignal has to_dict()
            "vehicles_crossed": {dir: self.vehicles[dir]['crossed'] for dir in ["up","down","left","right"]},
            "current_green": self.current_green,
            "simultaneous_green": self.simultaneous_green,
...
def is_green_for(intersection: Intersection, direction_number, lane=None, will_turn=None):
    ...
    if sim_dir is not None:
        if direction_number == sim_dir and will_turn == 1 and lane == 0:
            return False
    return False
...
intersection_left = Intersection("A", x_offset=0, entry_zones={
    'right': pygame.Rect(760, 411, 100, 200),
...
intersection_right = Intersection("B", x_offset=748, entry_zones={
    'left': pygame.Rect(440, 411, 100, 200),
...
def dynamic_signal_controller(inter: Intersection):
    ...
    while inter.SIGNAL_CONTROL_RUNNING:
        ...
        lane2duration = 0
        chosen_count_sim = 0
        for dir_name, wait in inter.wait_cycles.items():
            if wait >= STARVATION_LIMIT and remaining_counts[dir_name] > 0:
                forced_dir = dir_name
                chosen_count = remaining_counts[dir_name]
                break
        if forced_dir:
        ...
        if neighbor_id != 0 and neighbor_id != "0":
            ...
            if inter.name == 'A' and chosen_dir == 'left':
                ...
                print("Test A:", lane0duration, lane1duration, lane2duration)
                ...
                lane1duration = max(MIN_GREEN_DURATION, int(v1 * SECONDS_PER_VEHICLE))
                lane2duration = max(MIN_GREEN_DURATION, int(v2 * SECONDS_PER_VEHICLE))
                print("Test B:", lane0duration, lane1duration, lane2duration)
        ...
        # print("print duration", green_duration)
        ...
        for idx in [inter.current_green, inter.simultaneous_green]:
            ...
            inter.signals[idx].red = sum(
                ...
                if j not in [idx, inter.current_green, inter.simultaneous_green]
            )
...
def main(start_pygame=True, stop_flag=lambda: False):
    while not STOP_FLAG:
        global startup_time, startup_mode, time_elapsed, SIM_STARTED
